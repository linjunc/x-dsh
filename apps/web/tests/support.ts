 ...
 * lands in a workspace-less chat (startup auto-selection creates a session at
 * the Host cwd so the user can chat directly); every scenario that types into
 * a project workspace must connect one first. The hero's "Choose workspace"
 * chip button is the picker entry — present whether the session is the
 * workspace-less blank or no session at all. The directory is staged here
...
export async function connectFreshWorkspace(page: Page, root: string, name = 'workspace'): Promise<void> {
  ...
  // Startup lands in a workspace-less chat (no Workspace registered), so the
  // composer textarea is editable rather than a picker trigger. The hero's
  // "Choose workspace" chip button is the picker entry in either posture —
  // no-session or workspace-less blank — so click it to open the dialog.
