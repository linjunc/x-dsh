...
export interface HeroShellProps {
  ...
  t: HeroTranslate
  /** Custom headline; when empty the locale's shipped headline is used. */
  headline?: string
  /** Custom brand logo as a data URL; when empty the shipped FishLogo is used. */
  logo?: string
 ...
 */
export function HeroShell({ t, headline, logo, children }: HeroShellProps) {
  const text = headline !== undefined && headline.trim() !== '' ? headline : t('hero.headline')
  return (
    <div className={css.root}>
      <div className={css.stack}>
        <div className={css.headline}>
          {/* figma 34:10412: fish 34×25 leading the headline, gap 10. The
              custom brand logo replaces the shipped FishLogo when set. */}
          <span className={css.fishHitbox}>
            {logo !== undefined && logo !== ''
              ? <img className={css.heroLogo} src={logo} alt="" />
              : <FishLogo size={34} className={css.fish} />}
          </span>
          <span className={css.headlineText}>{text}</span>
        </div>
